...
// ------------------------------------------------------------
// ScoringTsPort (equivalente 1:1 de scoring.ts)
// ------------------------------------------------------------

object ScoringTsPort {

    private val lastMeteoModel: MutableMap<FishSpecies, MeteoModelInfo> = mutableMapOf()

    @RequiresApi(Build.VERSION_CODES.O)
    fun calculate(args: CalculateArgsInstant): ActivityScore {
        ...
        val now = args.context?.now ?: parseInstant(weather.time)
        ...
        val meteo = calculateMeteoScoreSpecies(
            weather = weather,
            derived = derived,
            ctx = MeteoContextInstant(
                now = now,
                anchorNow = args.context?.anchorNow,
                astro = astro,
        ...
        val astroScore = calculateAstroScoreSpecies(astro, now, normalizedSpecies, reasons)
        ...
        val baseWeights = Weights(
            meteo = 0.4,
            astro = 0.3,
            hydro = if (hydroScore != null) 0.2 else 0.0,
            marine = if (marineScore != null) 0.1 else 0.0
        )
        val sum = baseWeights.meteo + baseWeights.astro + baseWeights.hydro + baseWeights.marine
        val weightsUsed = ScoreBreakdown.WeightsUsed(
            meteo = if (sum > 0) baseWeights.meteo / sum else 0.0,
            astro = if (sum > 0) baseWeights.astro / sum else 0.0,
            hydro = if (sum > 0) baseWeights.hydro / sum else 0.0,
            marine = if (sum > 0) baseWeights.marine / sum else 0.0
        )
        ...
        val seasonFactor = seasonFactor(now, lat, normalizedSpecies)
        val spawnPenalty = spawnPenalty(now, lat, tempProxy, normalizedSpecies)
        val nightFactor = nightFactor(now, astro, normalizedSpecies)
    ...
    private fun calculateMeteoScoreSpecies(
        ...
        val dawnNorm = sunWindowNorm(ctx.now, ctx.astro, 90)
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun horizonHoursAhead(ctx: MeteoContextInstant): Double {
        val anchor = ctx.anchorNow ?: return 0.0
        val diffMs = ctx.now.toEpochMilli() - anchor.toEpochMilli()
        // Solo futuro
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun calculateAstroScoreSpecies(
        astro: AstroData,
        now: Instant,
        species: FishSpecies,
        reasons: MutableList<String>
    ): Double {
        val sunrise = parseInstant(astro.sunrise)
        val sunset = parseInstant(astro.sunset)

        val nowMs = now.toEpochMilli()
        val sunriseMs = sunrise.toEpochMilli()
        val sunsetMs = sunset.toEpochMilli()
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun seasonFactor(now: Instant, lat: Double, species: FishSpecies): Double {
        if (species == FishSpecies.general) return 1.0
        val month = monthUtc(now) // 1..12
        val northern = lat >= 0
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun spawnPenalty(now: Instant, lat: Double, waterTemp: Double, species: FishSpecies): Double {
        if (species == FishSpecies.general) return 1.0
        val month = monthUtc(now)
        val northern = lat >= 0
        val m = if (northern) month else ((month + 6 - 1) % 12) + 1
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun nightFactor(now: Instant, astro: AstroData, species: FishSpecies): Double {
    ...
    fun findBestWindowsDetails(
        ...
        // Usamos índices para detectar solapamientos (evita parseo de fechas y problemas de API level)
        ...
        // Selección voraz: ordenar por score descendente y elegir ventanas no solapadas (por índice)
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun sunWindowNorm(now: Instant, astro: AstroData, mins: Int = 90): Double {
        val sunrise = runCatching { parseInstant(astro.sunrise) }.getOrNull()
        val sunset = runCatching { parseInstant(astro.sunset) }.getOrNull()
        val ms = mins * 60_000L
        val nowMs = now.toEpochMilli()

        if (sunrise != null && abs(nowMs - sunrise.toEpochMilli()) <= ms) return 1.0
        if (sunset != null && abs(nowMs - sunset.toEpochMilli()) <= ms) return 1.0
        return 0.0
    }
    ...
    private fun parseTimeMs(iso: String): Long {
        // Acepta ISO8601 con offset, devuelve epoch millis
        return OffsetDateTime.parse(iso).toInstant().toEpochMilli()
    }

    @RequiresApi(Build.VERSION_CODES.O)
    private fun monthUtc(instant: Instant): Int =
        instant.atZone(ZoneOffset.UTC).monthValue
    ...
    private data class Weights(val meteo: Double, val astro: Double, val hydro: Double, val marine: Double)

    // Implementation-specific Instant-based context/args used by this variant
    data class ScoringContextInstant(
        val now: java.time.Instant? = null,
        val anchorNow: java.time.Instant? = null,
        val lat: Double = 0.0,
        val derived: DerivedWeatherFeatures? = null,
        val waterTemp: Double? = null
    )

    data class CalculateArgsInstant(
        ...
        val context: ScoringContextInstant? = null
    )

    private data class MeteoContextInstant(
